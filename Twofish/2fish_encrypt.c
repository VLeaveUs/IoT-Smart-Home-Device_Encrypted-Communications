...
int main(void){
	...
	//Input Text
	uint64_t P0, P1;
	P0 = 0x0123456789ABCDEF;
	P1 = 0x0123456789ABCDEF;

	uint32_t R0,R1,R2,R3,R4,R5,R6,R7,C2,C3,Temp1,Temp2;

	//Break It down
	R3 = P1 & (0x00000000FFFFFFFF);
	R2 = (P1 & (0xFFFFFFFF00000000)) >> 32;
	R1 = P0 & (0x00000000FFFFFFFF);
	R0 = (P0 & (0xFFFFFFFF00000000)) >> 32;
	...
	//Ciphertext
	Ciphertext[0] |= R4;
	Ciphertext[0] = (Ciphertext[0] << 32) | R5;
	Ciphertext[1] |= R6;
	Ciphertext[1] = (Ciphertext[1] << 32) | R7;

	printf("Ci0 = %I64u\nCi1 = %I64u", Ciphertext[0],Ciphertext[1]);
...
void F_func(uint32_t Fin0, uint32_t Fin1){
	...
	F[0] = ((c+K[2*r+8])%(0x100000000));
	F[1] = ((d+K[2*r+9])%(0x100000000));
}
